...
in the **child's** `_db.go` — no tags required on the parent.
...
type Role struct {
    ID     string
    UserID string  `db:"ref=user"`  // FK already declared — ormc reads this
    Name   string
}
```
...
2. `Role` has a field with `db:"ref=user"` — this is the FK back to `User`
3. → generates `ReadAllRoleByUserID` in `role_db.go`
...
| R3 | Loader generated in the **child's** `_db.go`, not the parent's | SRP: child struct owns its own query functions |
...
Pass 2 — For each StructInfo, resolve slice fields:
          if field.Type = "[]Role" and Role has db:"ref=user"
          → add RelationInfo to Role's StructInfo
          → generate ReadAllRoleByUserID in role_db.go
```
...
### Generated code (in `role_db.go`)

```go
// ReadAllRoleByUserID retrieves all Role records for a given UserID.
// Auto-generated by ormc — relation detected via db:"ref=user" on Role.UserID.
func ReadAllRoleByUserID(db *orm.DB, userID string) ([]*Role, error) {
    return ReadAllRole(db.Query(&Role{}).Where(Role_.UserID).Eq(userID))
}
```
...
func TestOrmc_RelationLoader(t *testing.T) {
    ...
    defer os.Remove("mock_generator_model_db.go")

    content, _ := os.ReadFile("mock_generator_model_db.go")
...
| Loader generated in child's `_db.go`, not parent's | ✅ |
